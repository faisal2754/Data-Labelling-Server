...
const { REGISTER, DELETE_JOB } = require('../graphql/mutations')
const prisma = require('../prisma/client')
const FormData = require('form-data')
const fs = require('fs')
const axios = require('axios').default
const { ok } = require('assert')
const { deletedJobs } = require('../schema/queries')
...
beforeAll(async () => {
   ...
   await new Promise((resolve) => app.listen({ port: 4009 }, resolve))
...
describe('Allow a user to accept a job', () => {
   ...
   it('should create a job', async () => {
      ...
      const res = await axios.post('http://localhost:4009/graphql', form, {
         headers
      })
