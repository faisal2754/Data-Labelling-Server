...
const { LABEL_JOB_INFO, COMPLETED_JOBS } = require('../graphql/queries')
const {
   REGISTER,
   ACCEPT_JOB,
   SAVE_STATE,
...
beforeAll(async () => {
   ...
   await new Promise((resolve) => app.listen({ port: 4011 }, resolve))
...
describe("It should return a job's results", () => {
   ...
   it('should create a job', async () => {
      ...
      const res = await axios.post('http://localhost:4011/graphql', form, {
         headers
      })
   ...
   it('should return the completed jobs', async () => {
      setOptions({
         request: {
            headers: {
               authorization: `Bearer ${jwt[0]}`
      ...
      const result = await query(COMPLETED_JOBS)
